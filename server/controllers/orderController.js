...
const calculateOrderCosts = async (cartItems, cartTotal, state) => {
  ...
  for (const item of cartItems) {
    ...
    // --- FIX: Check if product exists before accessing variants ---
    if (!product) continue; 
    // -------------------------------------------------------------
...
exports.createRazorpayOrder = async (req, res) => {
  try {
    ...
    // --- FIX: Filter out null/deleted products ---
    if (!cart || !cart.items?.length) {
      return res.status(400).json({ success: false, message: "No items in cart" });
    }
    ...
    if (validItems.length === 0) {
      return res.status(400).json({ success: false, message: "Cart contains only unavailable items" });
    }

    // Recalculate total strictly based on valid items
    const validTotal = validItems.reduce((acc, item) => acc + (item.price * item.quantity), 0);
    // ---------------------------------------------

    const costs = await calculateOrderCosts(validItems, validTotal, shippingState);
...
exports.verifyPayment = async (req, res) => {
  try {
    ...
    // --- FIX: Filter valid items again for security ---
    const validItems = cart.items.filter(item => item.product != null);
    if (validItems.length === 0) {
      return res.status(400).json({ success: false, message: "All items in cart are unavailable" });
    }
    const validTotal = validItems.reduce((acc, item) => acc + (item.price * item.quantity), 0);
    // --------------------------------------------------

    // Recalculate costs
    const costs = await calculateOrderCosts(validItems, validTotal, shippingAddress.state);
...
exports.calculateCosts = async (req, res) => {
  try {
    ...
    const cart = await Cart.findOne({ user: req.user.id })
      .populate({
        path: "items.product",
        select: "name variants weight weightUnit"
      });
    ...
    // --- FIX: Filter valid items and recalculate ---
    const validItems = cart.items.filter(item => item.product != null);
    if (validItems.length === 0) {
      return res.status(400).json({ success: false, message: "Cart contains only unavailable items" });
    }
