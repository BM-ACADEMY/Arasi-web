...
const Banner = () => {
  ...
  // --- HELPER: Safe Image URL Construction ---
  const getImageUrl = (imagePath) => {
    if (!imagePath) return null;
    if (imagePath.startsWith('http')) return imagePath; // It's already a full URL
    ...
    //    Example: "https://mysite.com/api" -> "https://mysite.com"
  ...
  // Use the helper to get the final URL
        ...
        >
          ...
          {imageUrl && (
            <img
              ...
              className="absolute inset-0 w-full h-full object-cover object-center z-0"
              onError={(e) => {
                // Optional: Hide image if it fails to load
                e.target.style.display = 'none';
                console.error("Banner image failed:", imageUrl);
              }}
            />
          )}
